...
swal.queue(steps).then((result) => {
  ...
  if (result.value) {
		...
		var siteCap = parseInt(result.value[0]);
		var targetPopulation = parseInt(result.value[1]);
		var perCapita = parseInt(result.value[2]);
...
bt.addEventListener('click', function() {
	...
	for(var i = 0; i < markers.length; i++)
	{
		...
		sitelist.push(can);
		sitelist[i].capacity=node_element.cap; // Add a new site in sitelist
		console.log(i + " - " + sitelist[i].capacity + "\n");
	}

	trx = ($('#tr').val()); // No of trucks
	trcap = ($('#tc').val()); // Truck Capacity
...
// Generic dfs
{
//	console.log('node = ' + node + " " + sitelist[node].adj.length + "\n");
	...
	currcap = currcap+sitelist[node].capacity; // Increases capacity of current truck by the capacity of the current node
	...
	//	console.log(next + " " + sitelist[next].visited + " " + currcap + " " + sitelist[next].capacity + " " + trcap + "\n");
...
{
	console.log("rdcnt/node: " + rdcnt + " " + node + "\n");
	...
	for (let i = 0; i < sitelist[node].adj.length; i++) // Iterate over adjancet nodes of current node sorted by heuristic
	{
		...
		console.log(rounds[rdcnt].cap + " " + sitelist[next].capacity + " " + trcap + "\n");
...
let build = () => {
	...
	for (let j = 0; j < sitelist[lfill].adj.length; j++) // Iterates over nodes connected to landfill
	{
		...
		if (i !== lfill && sitelist[i].visited === false) // If the node hasn't already been visited visit it
		{
			...
			var can = new Round([], 0, 0);// Initializes empty round to vector
			rounds.push(can);
...
let routingfunc = (n, m, lfill, trx, trcap, ...arr) => {
	...
	while (k < 100) // Running k from 0 to 100 for every 0.1
	{
		...
		k += 52; // Increase k by 0.1 - TO BE FIXED AGAIN, TESTING CHANGE
	}
	...
	console.log("Truks: " + trx + " " + trcap + "\n");

	for (let i = 0; i < trx; i++) {
		var emptrk=new Truck([], 0, 0);
		trucks.push(emptrk);
	}
	...
	{ // Notice that we want to minimize the time taken by the truck that takes maximum time
		...
		for (let j = 0; j < trx; j++) {
			console.log("Truck NO. " + j + " " + trucks[j].cumtm + "\n");
			if (trucks[j].cumtm < mn) // If current truck has less than mn cumalative time then set mn as this and make this minimum time truck
			{
				mni = j;
				mn = trucks[j].cumtm;
				console.log("cut min " + mni + "\n");
			}
		}

		console.log("current min " + mni + "\n");
